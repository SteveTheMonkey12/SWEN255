...
public class Game {
	...
	//Game State Machines
	  ...
	  public enum StatusGuess { Null, PlayerGuess, NextPlayerRespond }
	  private Status status;
	  private StatusGuess statusGuess;
	...
	// ------------------------
	  ...
	  public String getStatusFullName()
	  {
	    ...
	    if (statusGuess != StatusGuess.Null) { answer += "." + statusGuess.toString(); }
	    return answer;
	  }
	  ...
	  public StatusGuess getStatusGuess()
	  {
	    return statusGuess;
	  }

	  private boolean __autotransition3579__()
	  {
	  ...
	  public boolean guess()
	  {
	    ...
	    {
	      case Move:
	        setStatusGuess(StatusGuess.PlayerGuess);
	        wasEventProcessed = true;
	        break;
	      default:
	        // Other states do respond to this event
	  ...
	  public boolean incorrect(Player p)
	  {
	    boolean wasEventProcessed = false;
	  ...
	  public boolean makeGuess(Player p)
	  {
	    boolean wasEventProcessed = false;
	    
	    StatusGuess aStatusGuess = statusGuess;
	    switch (aStatusGuess)
	    {
	      case PlayerGuess:
	        exitStatusGuess();
	        // line 19 "model.ump"
	        tryGuess(p);
	        setStatusGuess(StatusGuess.NextPlayerRespond);
	        wasEventProcessed = true;
	        break;
	      default:
	        // Other states do respond to this event
	...
	public boolean respond(Player guessingPlayer, Player respondingPlayer, ArrayList<Item> guessedItems)
	  {
	    boolean wasEventProcessed = false;
	    ...
	    switch (aStatusGuess)
	    {
	      case NextPlayerRespond:
    	    if (respondingPlayer.equals(guessingPlayer))
	        {
	          exitStatus();
    	    ...
    	    for(Item guessedCard : guessedItems) {
		        if (respondingPlayer.getItems().contains(guessedCard))
		        {
		          exitStatus();
		          setStatus(Status.Solve);
		          wasEventProcessed = true;
	  ...
	  private void setStatus(Status aStatus)
	  {
	    ...
	    switch(status)
	    {
	      ...
	      case Move:
	        ...
	        break;
	      case Guess:
	        if (statusGuess == StatusGuess.Null) { setStatusGuess(StatusGuess.PlayerGuess); }
	        break;
	    }
	  }
	...
	private void exitStatusGuess()
	  {
	    switch(statusGuess)
	    {
	      case PlayerGuess:
	        setStatusGuess(StatusGuess.Null);
	        break;
	      case NextPlayerRespond:
	        setStatusGuess(StatusGuess.Null);
	        break;
	  ...
	  private void setStatusGuess(StatusGuess aStatusGuess)
	  {
	    statusGuess = aStatusGuess;
	    if (status != Status.Guess && aStatusGuess != StatusGuess.Null) { setStatus(Status.Guess); }
	  }
...
//				// distribute cards to player
//				players.clear();
//				cardNumbers = items.size() / numPlayers;
//				for (int i = 0; i < numPlayers; i++) {
...
//				}
	...
	private void tryGuess(Player p) {
		// TODO Auto-generated method stub
