...
public class Game {
	...
	public boolean addWeaponAt(Weapon aWeapon, int index) {
		...
		if (addWeapon(aWeapon)) {
			...
			if (index > numberOfWeapons()) {
				index = numberOfWeapons() - 1;
			}
			weapons.remove(aWeapon);
	...
	public boolean addEstateAt(Estate aEstate, int index) {
		...
		if (addEstate(aEstate)) {
			...
			if (index > numberOfEstates()) {
				index = numberOfEstates() - 1;
			}
			estates.remove(aEstate);
	 ...
	 * @return
	 */
	private Card playerGuessResponse(Player aPlayer, Character guessedPlayer, Weapon guessedWeapon,
	...
	/**
	 * initial the cards of estate, character and weapon. randomly choose the murder
	 * cards. distribute cards to player leave the board part at this stage (to be
	 * continued...)
	 */
	public void initial() {
	...
	private int solveAttempt(Player p) {
		...
		if (!guess.equals("y") && !guess.equals("n")) {
			...
			if (murderCards.contains(guessedWeapon) && murderCards.contains(guessedEstate)
					&& murderCards.contains(guessedPlayer)) {
