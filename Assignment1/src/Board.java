...
// line 11 "model.ump"
	...
	public boolean moveCharacter(Character target, String cardinalDirection) {
		...
		// if they have a location move them to the doorway the intend to leave via and
		// then continue with the validity checking
		...
		for (Character p : this.characters) {
			...
			if (this.characterPositions.get(p).equals(newPos)
					&& this.characterPositions.get(target).getLocation() == null) {
				return false;
			}
		}
	...
	public String toString() {
		...
		for (Map.Entry<Character, Position> entry : this.characterPositions.entrySet()) {
			...
			if (entry.getValue().getLocation() == null) {
				board[entry.getValue().getX()][entry.getValue().getY()] = "|"
